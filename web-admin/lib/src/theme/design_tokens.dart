...
/// `docs/design-system/design-tokens.json` (igual que hizo el cliente móvil).
/// Esta clase resuelve las referencias semánticas
/// (p.ej. `color.semantic.primary -> color.brand.rotary_royal_blue`) y expone
/// getters tipados. Ningún widget de la web admin debe declarar colores/medidas
/// literales: deben pedirlos a [DesignTokens]. Reimplementación local (no se
/// importa código de `mobile/`).
class DesignTokens {
  ...
  String get fontFamilyBase {
    final value =
        (_section('typography')['font_family_base'] as Map)['value'] as String;
  ...
  TypeToken typeToken(String name) {
    final scale =
        (_section('typography')['scale'] as Map).cast<String, dynamic>();
  ...
  double radius(String name) => ((_section('radius')[name]) as num).toDouble();
