"""Vista restringida — coords **exactas**, SOLO rol ``aliado_firmante`` (gate #5).

La justificación es protección del árbol (tala/vandalismo), no privacidad del voluntario.
"""
...
def restricted_observations(
    ...
    user: CurrentUser = Depends(require_role("aliado_firmante")),
    db: Session = Depends(get_db),
) -> list[RestrictedObservation]:
    """Coords EXACTAS. Requiere rol aliado_firmante (gate #5)."""
